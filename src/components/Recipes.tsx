            ...
            <div className="flex flex-wrap gap-2">
             {
                Object.keys(tasteColorMapping).map(taste => {
                    ...
                    />
                    <span className={`p-2 text-xs font-bold border border-b-4 border-r-4 border-black rounded-lg shadow-xs hover:shadow-sm flex justify-center items-center ${isChecked ? tasteColorMapping[taste as TasteColorMapping] : `bg-white`}`}>{taste}
                     {/* ({tasteCount[taste] || 0}) */}
                     </span>
                  </label>
                })
            ...
            <ul className="flex flex-col gap-6">
                {
                    filteredResult.map(({ name, tastes, ingredients, score }) => {
                        return <a key={name} href={`/mixologist/cocktails/${name.toLocaleLowerCase().replace(/ /g, "-")}`}>
                            <div className={`p-4 border border-b-4 border-r-4 border-black rounded-lg shadow-xs hover:shadow-sm 
                            ${tasteColorMapping[tastes.length && tastes[0] as TasteColorMapping]} ${score < 1 ? `opacity-60`: ``}`}
                            >
                                ...
                                <ul>
                                    ...
                                    if ($ingredientsInventory.includes(name.toLocaleLowerCase())) {
